...
const AnonymousDeposits: React.FC<AnonymousDepositsProps> = ({ transactions, setTransactions, customers }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {anonymousList.map(t => (
          <div key={t.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all overflow-hidden relative text-right">
            ...
            <p className="text-xs text-slate-500 mt-4 italic min-h-[40px] leading-relaxed text-right">"{t.description}"</p>
            <div className="mt-8">
          ...
          <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-xl shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="space-y-6 text-right">
              <div className="grid grid-cols-2 gap-4 text-right">
                <input type="number" placeholder="مبلغ" className="p-5 bg-slate-50 rounded-2xl text-xl font-black border border-slate-100 text-right" value={newAnon.amount || ''} onChange={e => setNewAnon({...newAnon, amount: Number(e.target.value)})} />
                <select className="p-5 bg-slate-50 rounded-2xl font-black border border-slate-100 text-right" value={newAnon.currency} onChange={e => setNewAnon({...newAnon, currency: e.target.value})}>
                  {SUPPORTED_CURRENCIES.map(c => <option key={c.code} value={c.code}>{c.code}</option>)}
                </select>
              </div>
              <textarea placeholder="توضیحات یا نشانه..." className="w-full p-5 bg-slate-50 rounded-2xl text-sm font-bold min-h-[100px] border border-slate-100 text-right" value={newAnon.description} onChange={e => setNewAnon({...newAnon, description: e.target.value})} />
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-50 flex items-center justify-center p-4 text-right">
          <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-3xl h-[80vh] flex flex-col text-right">
            <h3 className="text-3xl font-black mb-8 text-right">انتخاب صاحب وجه</h3>
            <input type="text" placeholder="جستجوی نام مشتری..." className="w-full bg-slate-50 p-6 rounded-3xl mb-6 font-bold text-right" value={customerSearch} onChange={e => setCustomerSearch(e.target.value)} />
            <div className="flex-1 overflow-y-auto space-y-3 text-right">
              {filteredCustomers.map(c => (
                <button key={c.id} onClick={() => handleAssignToCustomer(c)} className="w-full p-6 bg-slate-50 rounded-3xl flex justify-between items-center hover:bg-blue-600 hover:text-white transition-all text-right">
