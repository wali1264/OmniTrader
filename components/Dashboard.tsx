    ...
    <div className="space-y-6 fade-entry">
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">System Operational Status: <span className="text-slate-200">Manual Control Mode</span></p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <div className="flex items-baseline gap-1 tnum">
              <h4 className="text-xl font-black text-blue-700">{(globalRates.find(r => r.currencyCode === 'USD')?.rateToAfn || 0).toFixed(4)}</h4>
            </div>
            <button onClick={fetchLatestRates} className="mt-4 w-full bg-slate-50 hover:bg-slate-100 border border-slate-200 py-2 rounded-lg text-[9px] font-bold transition-all text-slate-600">
              UPDATE RATE
            </button>
        </div>
        ...
        <StatCard title="صندوق دالر (USD)" value={(stats.cashBox['USD'] || 0).toLocaleString()} unit="USD" icon={<DollarSign size={14} className="text-blue-500" />} />
        <StatCard title="صندوق کلدار (PKR)" value={(stats.cashBox['PKR'] || 0).toLocaleString()} unit="PKR" icon={<Coins size={14} className="text-purple-500" />} />
        <StatCard title="صندوق ایرو (EUR)" value={(stats.cashBox['EUR'] || 0).toLocaleString()} unit="EUR" icon={<Coins size={14} className="text-indigo-500" />} />
        ...
        <StatCard title="تومان نقدی" value={(stats.cashBox['IRT_CASH'] || 0).toLocaleString()} unit="IRT" icon={<Coins size={14} className="text-amber-500" />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {bankAccountBalances.map(account => (
              <div key={account.id} className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex justify-between items-center transition-all hover:bg-white hover:border-slate-200">
                <div>
                   ...
                   <p className="text-[9px] font-mono text-slate-400 mt-1 uppercase">Acc: {account.accountNumber}</p>
                </div>
                <div className="text-left">
        ...
        <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4 text-right">
           <h3 className="text-[10px] font-bold text-slate-900 border-b border-slate-50 pb-3 uppercase tracking-widest">Daily Summary (USD)</h3>
      ...
      <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex justify-between items-center mb-8">
           <h3 className="text-[10px] font-bold text-slate-800 uppercase tracking-widest">Financial Flow Analytics</h3>
           <div className="flex gap-4">
              <div className="flex items-center gap-2">
                 <div className="w-3 h-3 rounded bg-blue-500"></div>
                 <span className="text-[9px] font-bold text-slate-400 uppercase">Resid</span>
              </div>
              <div className="flex items-center gap-2">
                 <div className="w-3 h-3 rounded bg-rose-500"></div>
                 <span className="text-[9px] font-bold text-slate-400 uppercase">Board</span>
              </div>
           </div>
        </div>
