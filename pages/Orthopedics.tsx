...
import { Bone, User, Upload, Activity, AlertCircle, CheckCircle, Scale } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex bg-white rounded-2xl p-2 shadow-sm border border-gray-100 max-w-2xl">
        <button
          onClick={() => { setActiveTab('posture'); setResult(null); setImage(null); setPreview(null); }}
          className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'posture' ? 'bg-orange-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <User /> اسکنر پاسچر (ستون فقرات)
        </button>
        <button
          onClick={() => { setActiveTab('joints'); setResult(null); setImage(null); setPreview(null); }}
          className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'joints' ? 'bg-orange-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <Bone /> آنالیز مفاصل (آرتروز)
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 h-fit">
           <h3 className="font-bold text-gray-800 mb-2">
             {activeTab === 'posture' ? 'آپلود تصویر تمام‌قد ایستاده' : 'آپلود تصویر مفصل دردناک'}
           </h3>
           <p className="text-sm text-gray-500 mb-6">
             {activeTab === 'posture' 
                ? 'برای تشخیص قوز، گودی کمر و انحراف ستون فقرات، یک عکس تمام قد از پهلو یا پشت بگیرید.' 
                : 'برای بررسی تورم، قرمزی و تغییر شکل، از نزدیک از مفصل (زانو، دست، پا) عکس بگیرید.'}
           </p>
           ...
           <div className="border-2 border-dashed border-orange-200 bg-orange-50/30 rounded-2xl h-80 flex flex-col items-center justify-center relative overflow-hidden group">
              ...
              {preview ? (
                <img src={preview} className="w-full h-full object-contain" alt="Orthopedic" />
              ) : (
                <div className="text-center p-4">
                  <div className="w-16 h-16 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                     {activeTab === 'posture' ? <User size={32} /> : <Bone size={32} />}
                  </div>
           ...
           <button 
             ...
             className="w-full mt-6 bg-orange-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-orange-200 hover:bg-orange-700 disabled:opacity-50"
           >
             {loading ? 'در حال ترسیم خطوط تراز...' : 'آنالیز ساختار اسکلتی'}
           </button>
        </div>
        ...
        <div className="space-y-6">
           {result ? (
             <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-200 animate-fade-in">
                <div className={`p-6 text-white flex justify-between items-center ${
                  ...
                  result.severity === 'concern' ? 'bg-orange-500' : 'bg-green-600'
                   ...
                   <div>
                     <h3 className="text-xl font-bold">نتیجه ارزیابی</h3>
                ...
                <div className="p-6 space-y-6">
                     ...
                     <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {result.angles.map((a, i) => (
                             <div key={i} className="bg-white p-2 rounded-lg text-sm text-gray-700 shadow-sm border border-orange-100 text-center font-mono">
                               {a}
                             </div>
                          ))}
                        </div>
                   ...
                   <div>
                      <h4 className="font-bold text-gray-800 mb-3 border-b pb-2">یافته‌های ساختاری</h4>
                   ...
                   <div className="bg-blue-50 p-4 rounded-xl">
                      <h4 className="font-bold text-blue-800 mb-2 flex items-center gap-2">
                         <Activity size={18} />
                         حرکات اصلاحی و ورزش‌ها (Rehab)
                      </h4>
                      <ul className="space-y-2">
                        {result.recommendations.map((r, i) => (
                          <li key={i} className="text-sm text-blue-900 bg-white p-2 rounded-lg border border-blue-100">
                             {r}
                          </li>
                        ))}
                      </ul>
             ...
             <div className="h-full bg-gray-100 rounded-3xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 p-8 text-center opacity-70">
                <Bone size={48} className="mb-4" />
                <p>منتظر تصویر برای آنالیز بیومکانیک...</p>
             </div>
           )}
        </div>
      </div>
