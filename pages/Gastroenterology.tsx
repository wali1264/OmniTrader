...
import { Utensils, Microscope, Activity, AlertCircle, CheckCircle, Upload, MapPin, Flame } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="flex bg-white rounded-2xl p-2 shadow-sm border border-gray-100 max-w-3xl">
        <button
          onClick={() => { setActiveTab('meal'); setResult(null); setImage(null); setPreview(null); }}
          className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'meal' ? 'bg-emerald-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <Utensils /> اسکنر غذا و مزاج
        </button>
        <button
          onClick={() => { setActiveTab('endoscopy'); setResult(null); setImage(null); setPreview(null); }}
          className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'endoscopy' ? 'bg-emerald-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <Microscope /> آندوسکوپی هوشمند
        </button>
        <button
          onClick={() => { setActiveTab('pain'); setResult(null); }}
          className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'pain' ? 'bg-emerald-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <MapPin /> نقشه درد شکمی
        </button>
      </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 h-fit">
          ...
          {(activeTab === 'meal' || activeTab === 'endoscopy') && (
            <div className="space-y-6">
              <h3 className="font-bold text-gray-800">
                {activeTab === 'meal' ? 'آپلود عکس غذا' : 'آپلود تصویر آندوسکوپی/کولونوسکوپی'}
              </h3>
              <p className="text-sm text-gray-500">
                {activeTab === 'meal' 
                  ? 'برای محاسبه کالری و تشخیص طبع (سرد/گرم) غذا.' 
                  : 'برای تشخیص زخم، پولیپ، و التهابات گوارشی.'}
              </p>
              ...
              <div className="border-2 border-dashed border-emerald-200 bg-emerald-50/30 rounded-2xl h-80 flex flex-col items-center justify-center relative overflow-hidden group">
                 ...
                 {preview ? (
                   ...
                   <div className="text-center p-4">
                     {activeTab === 'meal' ? <Utensils className="mx-auto text-emerald-400 w-12 h-12 mb-3" /> : <Microscope className="mx-auto text-emerald-400 w-12 h-12 mb-3" />}
          ...
          {activeTab === 'pain' && (
            <div className="space-y-6">
               ...
               <div className="mt-4">
                 <label className="text-sm font-bold text-gray-700 block mb-2">علائم همراه</label>
                 <input 
                   type="text" 
                   className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500" 
                   placeholder="مثال: تهوع، تب، نفخ..."
                   value={painSymptoms}
          ...
          <button 
            ...
            className="w-full mt-6 bg-emerald-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-700 disabled:opacity-50"
          >
            {loading ? 'در حال تحلیل گوارشی...' : 'شروع آنالیز'}
          </button>
        </div>
        ...
        <div className="space-y-6">
           {result ? (
                ...
                <div className="p-6 space-y-6">
                   ...
                   {result.nutrients && result.nutrients.length > 0 && (
                      <div className="bg-gray-50 p-4 rounded-xl">
                        <h4 className="font-bold text-gray-700 mb-2">ارزش غذایی</h4>
                        <div className="flex flex-wrap gap-2">
                           {result.nutrients.map((n, i) => (
                              <span key={i} className="bg-white px-3 py-1 rounded-full text-xs text-gray-600 border border-gray-200 shadow-sm">{n}</span>
                           ))}
                        </div>
                      </div>
                   )}
                   ...
                   {result.organ && (
                      <div className="bg-red-50 p-4 rounded-xl border border-red-100 text-center">
                         <span className="text-xs text-red-500 font-bold uppercase">عضو درگیر احتمالی</span>
                         <p className="text-2xl font-bold text-red-800">{result.organ}</p>
                      </div>
                   )}
                   ...
                   <div className="bg-emerald-50 p-4 rounded-xl">
                      <h4 className="font-bold text-emerald-800 mb-2">توصیه‌ها</h4>
             ...
             <div className="h-full bg-gray-100 rounded-3xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 p-8 text-center opacity-70">
                ...
                <p>منتظر داده‌ها برای آنالیز گوارش...</p>
             </div>
           )}
        </div>
      </div>
